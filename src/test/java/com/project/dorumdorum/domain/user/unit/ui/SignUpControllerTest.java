...
class SignUpControllerTest {
    ...
    void signUp_CallsUseCaseAndReturnsSuccess() {
        ...
        when(signUpUseCase.execute(request)).thenReturn("u1");

        ResponseEntity<Void> response = controller.signUp(request);

        verify(signUpUseCase).execute(request);
        assertThat(response.getStatusCode()).isEqualTo(HttpStatus.CREATED);
        assertThat(response.getHeaders().getLocation()).isEqualTo(URI.create("/api/users/u1/profile"));
